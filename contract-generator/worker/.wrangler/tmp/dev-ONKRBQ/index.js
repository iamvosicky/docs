...
// .wrangler/tmp/bundle-6ybItQ/checked-fetch.js
...
app.post("/", authMiddleware, adminMiddleware, async (c) => {
  try {
    ...
    const schema_json = formData.get("schema_json");
...
app.delete("/:id", authMiddleware, adminMiddleware, async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      "INSERT INTO AuditLog (actor_id, action, obj_id, ts) VALUES (?, ?, ?, ?)"
    ).bind(c.get("user").id, "TEMPLATE_DELETED", id, (/* @__PURE__ */ new Date()).toISOString()).run();
    return c.json({
...
// src/routes/files.ts
var app2 = new Hono2();
app2.post("/generate/:templateId", authMiddleware, async (c) => {
  ...
  try {
    ...
    await c.env.PDF_QUEUE.send({
      fileId,
      docxUrl: await c.env.STORAGE.createSignedUrl(docxKey, 3600)
      // 1 hour expiry
    });
    const docxUrl = await c.env.STORAGE.createSignedUrl(docxKey, 86400);
    return c.json({
      ...
      file: {
        ...
        docxUrl,
        pdfUrl: null
        // Will be available after PDF conversion
      }
    });
  } catch (error) {
...
app2.get("/", authMiddleware, adminMiddleware, async (c) => {
  try {
    const { results } = await c.env.DB.prepare(
      "SELECT f.id, f.template_id, f.user_id, f.created_at, t.name as template_name, u.email as user_email FROM Files f JOIN Templates t ON f.template_id = t.id JOIN Users u ON f.user_id = u.id ORDER BY f.created_at DESC"
    ).all();
...
app2.get("/:id", authMiddleware, async (c) => {
  ...
  try {
    ...
    const docxUrl = await c.env.STORAGE.createSignedUrl(file.docx_key, 86400);
    let pdfUrl = null;
    if (file.pdf_key) {
      pdfUrl = await c.env.STORAGE.createSignedUrl(file.pdf_key, 86400);
    }
    return c.json({
      file: {
        ...file,
...
app2.delete("/:id", authMiddleware, adminMiddleware, async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      "INSERT INTO AuditLog (actor_id, action, obj_id, ts) VALUES (?, ?, ?, ?)"
    ).bind(user.id, "FILE_DELETED", id, (/* @__PURE__ */ new Date()).toISOString()).run();
    return c.json({
...
app3.route("/api/templates", templateRoutes);
app3.route("/api/files", fileRoutes);
app3.get("/", (c) => {
  return c.json({
...
// .wrangler/tmp/bundle-6ybItQ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6ybItQ/middleware-loader.entry.ts
